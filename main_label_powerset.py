...
trans_result = lp_svm.transform(y_train_train)
y_trans = trans_result['train_vector']
inverse_result = lp_svm.inverse_transform(y_trans, trans_result)

# Fit using lp
lp_fit = lp_svm.fit(X_train_train,trans_result)
lp_pred = lp_svm.predict(lp_fit ,X_train_test,trans_result)
...
trans_result = lp_logistic.transform(y_train_train)
y_trans = trans_result['train_vector']
inverse_result = lp_logistic.inverse_transform(y_trans, trans_result)

# Fit using lp
lp_fit = lp_logistic.fit(X_train_train,trans_result)
lp_pred = lp_logistic.predict(lp_fit ,X_train_test,trans_result)
