...
def fit(X, trans_result,cluster):
    ...
    for i in range(len(cluster)):
        classifier = lp_svm.fit(X, trans_result[i])
...
def predict(classifiers, X, trans_result):
    new_X = X    
    predictions = [csc_matrix(np.array(lp_svm.predict(classifiers[i], new_X, trans_result[i]))) for i in range(len(classifiers))]
